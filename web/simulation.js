...
function jR(U) {
    let d = new Set(["Jevton", "Jev", "Council", "Gazette"]);
...
function MP(U) {
    ...
    for (let [T, R] of [...D.entries()].sort((J, Q) => J[0] - Q[0])) {
        let J = T * 13,
            ...
            B = T === 0 ? ld([`This is Jevton, a town of ${U.residents.length} in the ${M}. `, `Jevton, ${M}: ${U.residents.length} residents, seven roads, and a council that lets Jev decide. `], J) : ld([`Day ${T+1} began in ${M}, ${S}. `, `The ${M} sun rose on day ${T+1} to ${S}. `, `Day ${T+1}. ${S[0].toUpperCase()}${S.slice(1)}, and the town went about its business. `], J),
