...
export const payroll=(t,id)=>t.residents.filter(r=>r.alive&&r.work===id).reduce((n,r)=>n+Math.round(wage[r.job]*(r.wageFactor||1)),0);
...
export function valuation(t,id){
  ...
  const observed=(revenue*2+Math.max(0,profit)*10)*clamp(1+(growth||0)*.35,.65,1.5)*(profit<0?.7:1)*(staff?1:.25);
...
export function portfolio(t,i){
  ...
  const sales=t.finance.ledger.filter(x=>x.kind==='liquidation'&&x.to===i.id);
  const realized=Number.isFinite(i.realized)?i.realized:sales.length&&sales.every(x=>Number.isFinite(x.removedBasis))?sales.reduce((sum,x)=>sum+x.amount-x.removedBasis,0):null;
  return {value,cost,unrealized:value-cost,realized,netWorth:i.cash+value,totalReturn:i.cash+value-i.initialCash};
}
export const signedMoney=n=>Number.isFinite(n)?`${Math.round(n)>0?'+':Math.round(n)<0?'−':''}${money(Math.abs(Math.round(n)))}`:'—';
export const saleOutcome=realized=>Math.round(realized)>0?'profit':Math.round(realized)<0?'loss':'even';
export const pendingPitches=t=>t.finance.offers.filter(o=>o.status==='queued'&&!o.playerPitchDecision);
export const waitingForPlayer=t=>t.finance.playerActive&&!!(t.finance.active||t.finance.offers.some(o=>['queued','open'].includes(o.status)));
export function decidePitches(t,{lotId,lotIds,action}){
  check(!t.finance.active,'Finish the active auction first');
  check(['take','pass'].includes(action),'Unknown pitch decision');
  const ids=[...new Set(lotIds||[lotId])];
  check(action==='pass'||ids.length===1,'Take one pitch at a time');
  // Snapshot first. A later pitch is never implicitly covered by Pass all.
  const lots=ids.map(id=>t.finance.offers.find(o=>o.id===id));
  check(lots.length&&lots.every(o=>o?.status==='queued'&&!o.playerPitchDecision),'This pitch has already been decided');
  for(const lot of lots){lot.playerPitchDecision={action,hour:t.hour};lot.playerDeclined=action==='pass';}
  return ids;
}
...
export function investmentCase(t,id){
  const v=valuation(t,id),b=t.businesses[id],h=b.capital.history,e=b.economy;
  const recent=h.slice(-6),dailyRecent=recent.reduce((n,x)=>n+x.revenue,0)*24/Math.max(1,recent.length),dailyTrailing=v.revenue*24/Math.max(1,v.hours);
  const momentum=dailyTrailing>0?clamp(dailyRecent/dailyTrailing-1,0,.6):0;
  const profitable=v.hours>=6&&v.profit>0&&v.revenue>0;
  const margin=profitable?clamp(v.profit/v.revenue,0,.4):0;
  const delivery=profitable&&e.plan?.stage==='implementing'&&e.plan.ready<=t.hour+12?.2*e.owner.discipline/100:0;
  const upside=profitable?Math.min(.65,momentum*.6+margin*.5+delivery):0;
  return {current:v.value,outlook:Math.round(v.value*(1+upside)),upside,hours:v.hours,profitable};
}
export function fairLotValue(t,lot){const b=t.businesses[lot.businessId],supply=shareCount(b)+(lot.kind==='primary'&&['queued','open'].includes(lot.status)?lot.shares:0);return investmentCase(t,lot.businessId).outlook*lot.shares/supply;}
...
const qSale={type:'choice',instructions:'Review this holding for this hour: hold, sell half, or sell the entire stake at the stated marked cash quote?',criteria:{sell:'Sell half to realize some gains, reduce exposure or recycle capital.',exit:'Sell the entire remaining stake and recycle the cash.',hold:'Keep the equity for future value growth; no dividends are paid.'}};
...
async function prepare(t,judge){
  ...
  const reviews=[];
  for(const i of t.investors.filter(i=>!i.human&&t.hour-i.lastSaleReview>=1)){
    i.lastSaleReview=t.hour;
    for(const [id,b]of Object.entries(t.businesses))if((b.capital.holdings[i.id]||0)>=1)reviews.push({i,id});
  }
  if(reviews.length){const results=await ask(t,judge,reviews.map(({i,id},n)=>({id:`sale${n}`,state:`${i.name}. ${i.strategy} Cash ${money(i.cash)}; reserve ${money(i.reserve)}. Holding estimated ${money(ownership(t.businesses[id],i.id)*valuation(t,id).value)}, remaining cost ${money(i.basis[id]||0)}. Immediate half-stake quote ${money(liquidationQuote(t,i,id,Math.max(1,Math.floor(t.businesses[id].capital.holdings[i.id]/2))))}; full exit ${money(liquidationQuote(t,i,id,t.businesses[id].capital.holdings[i.id]))}. Quotes equal current marked value. 24h business earnings outlook ${money(investmentCase(t,id).outlook)}; selling now gives up that uncertain upside. ${businessFacts(t,id)}`,questions:{decision:qSale}})),'share liquidation');
    for(const [{i,id},n]of reviews.map((x,n)=>[x,n])){const a=results[`sale${n}`].decision;i.decisions++;i.log.push({hour:t.hour,businessId:id,phase:'liquidation',...a});const held=t.businesses[id].capital.holdings[i.id];if(a.choice==='exit')liquidate(t,i,id,held);else if(a.choice==='sell')liquidate(t,i,id,Math.max(1,Math.floor(held/2)));}}
...
export async function runAuction(t,lot,judge,{onUpdate=()=>{},humanTurn}={}){
  ...
  for(let count=0;count<500;count++){
    ...
    if(i.human){check(humanTurn,'Player turn needs a human decision handler');action=await humanTurn(lot,i);check(['bid','pass'].includes(action),'Unknown player action');}
      ...
      const a=(await ask(t,judge,[{id:i.id,state:`${i.name}: ${i.strategy} Available cash ${money(i.cash)}, reserve ${money(i.reserve)}. ${businessFacts(t,lot.businessId)} Fixed lot ${(lot.equity*100).toFixed(1)}% equity; 24h lot outlook ${money(fairLotValue(t,lot))} (uncertain, based on profitable growth and funded plans). Next bid ${money(amount)}; current high ${money(lot.highBid)}. Cash funds the business; unspent new capital is excluded from its estimated value. No dividends. Current exposure cost ${money(i.basis[lot.businessId]||0)}. Winning cash is committed, returns uncertain.`,questions:{decision:qBid}}],'auction bid'))[i.id].decision;
...
export async function financeTurn(t,judge,options={}){
  ...
  for(const lot of t.finance.offers.filter(o=>o.status==='queued'&&(!options.lotId||o.id===options.lotId)&&(!options.lotIds||options.lotIds.includes(o.id))).slice(0,options.maxLots||3)){await runAuction(t,lot,judge,options);if(lot.status==='funded')funded.push(lot);}
