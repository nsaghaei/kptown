import {hoursLabel,isPlaceOpen} from './hours.js';
import {money,ownership,shareCount,joinPlayer,portfolio,valuation,nextPrice,bidLimit,liquidate,liquidationQuote,signedMoney,saleOutcome,pendingPitches,investmentCase} from './investors.js';
...
export function installUI({getTown,isBusy,runPlaza,refresh,onJoin,onWin,onSale,focusBusiness,focusPlaza,onQueueUpdate}){

  let view={type:'queue'},pending=null,lastFocused=null,collapsed=false,shown=false;const celebrated=new Set(),detailScroll=new Map();let closingTimer=null,lastRenderedView=null;
  ...
  const queued=()=>pendingPitches(getTown());
  ...
  function renderPortfolio(){const oldScroll=portfolioCard.scrollTop,holdScroll=portfolioCard.querySelector('.portfolio-holdings')?.scrollTop||0,pitchScroll=portfolioCard.querySelector('.pitch-list')?.scrollTop||0;const t=getTown(),p=t.investors.find(i=>i.human),count=queued().length;portfolioCard.replaceChildren(el('h3',p?.name||'Your investment firm'));
  ...
  function history(id){const b=getTown().businesses[id];return details('Hourly performance history',table(['Hour','Gross','Refunds','Net','Profit','Purchases','Period'],b.capital.history.slice(-24).reverse().map(h=>[h.hour,money(h.grossRevenue),money(h.refunds||0),money(h.revenue),money(h.profit),h.customers,h.skipped?'Sleep skipped':'Simulated'])));}

  function businessContent(id){const t=getTown(),b=t.businesses[id],p=t.places.find(p=>p.id===id),e=b.economy,box=el('div',undefined,'business-evidence');const type=p.venue||p.serviceRole||p.category||p.kind;box.append(el('p',`${p.name} is ${type==='office'?'an':'a'} ${type==='books'?'bookstore':type==='clothing'?'clothing store':type==='leisure'?'leisure venue':type} in ${p.neighborhood||'the town'}. It offers ${productNames[e.product].toLowerCase()}. ${e.failed?'The business has closed.':`${t.residents.filter(r=>r.alive&&r.work===id).length} residents work here.`}`,'business-bio'));

    box.append(table(['Business plan','Current state'],[['Opening hours',`${hoursLabel(p)} · ${isPlaceOpen(p,t.hour)?'Open':'Closed'}`],['Funding purpose',e.plan?`${e.plan.label} · ${money(e.plan.cost)} · ${e.plan.stage}`:'Working capital / no active upgrade'],['Owner adaptability / execution',`${e.owner.adaptability}/100 · ${e.owner.discipline}/100`],['Capacity / appeal',`${e.capacity} per hour · ${e.appeal.toFixed(2)}`]]),el('h3','Performance scorecard'),metricsTable(id),history(id),details('Ownership & capital',table(['Holder','Shares','Ownership'],[['Founders',b.capital.founderShares,pct(b.capital.founderShares/shareCount(b))],...Object.entries(b.capital.holdings).filter(([,n])=>n>0).map(([iid,n])=>[name(iid),n,pct(n/shareCount(b))]),['Settlement account',b.capital.marketShares,pct(b.capital.marketShares/shareCount(b))]]),table(['Balance-sheet item','Value'],[['Cash',money(b.till)],['Unspent financing excluded from mark',money(b.capital.restricted)],['Total equity raised',money(b.capital.raised)],['Unpaid refunds',money(scorecard(t,id).refundDue)]])),details('Owner execution history',table(['Hour','Event','Action','Product'],e.history.slice(-20).reverse().map(h=>[h.hour,h.kind,h.type||h.strategy||'',h.product||'']))));return box;}
  ...
  function auctionContent(lot){const box=el('div',undefined,'auction-bidding');box.append(el('h3','Live bidding'),table(['Auction','Current'],[['Fixed stake',`${pct(lot.equity)} · ${lot.shares} shares`],['Opening / next bid',`${money(lot.reserve)} / ${money(nextPrice(lot))}`],['High bid',lot.highBid?`${money(lot.highBid)} · ${name(lot.highBidder)}`:'No bid yet'],['Next turn',lot.next?name(lot.next):lot.status],['Result',lot.status==='funded'?`${name(lot.investorId)} won for ${money(lot.amount)}`:lot.status]]));
    ...
    if(pending?.lot.id===lot.id){const controls=el('div',undefined,'bid-controls');controls.append(el('strong','Your turn'));const bid=button(`Bid ${money(nextPrice(lot))}`,()=>resolve('bid'),'btn primary');bid.disabled=nextPrice(lot)>bidLimit(getTown(),pending.investor,lot);controls.append(bid,button('Pass this auction',()=>resolve('pass')));box.append(controls);}return box;}
  ...
  function render(){if(lastRenderedView)detailScroll.set(lastRenderedView,[...dock.querySelectorAll('.business-body,.business-evidence,.auction-bidding,.bid-scroll')].map(node=>[node.className,node.scrollTop]));renderPortfolio();onQueueUpdate?.(getTown());if(shown){if(closingTimer){clearTimeout(closingTimer);closingTimer=null;}dock.classList.remove('leaving');dock.classList.add('entering');dock.hidden=false;}else if(!dock.hidden&&!closingTimer){dock.classList.remove('entering');dock.classList.add('leaving');closingTimer=setTimeout(()=>{closingTimer=null;if(!shown){dock.hidden=true;dock.classList.remove('leaving');}},320);} app.classList.toggle('business-open',shown);app.classList.toggle('business-collapsed',collapsed);if(!shown)return;
    ...
    const body=el('div',undefined,view.type==='business'&&lot?'business-body with-auction':'business-body');if(view.type==='business'){body.append(businessContent(view.id));if(lot)body.append(auctionContent(lot));}else body.append(view.type==='investor'?investorContent(view.id):queueContent());body.hidden=collapsed;dock.replaceChildren(header,body);lastRenderedView=`${view.type}:${view.id||''}:${view.lotId||''}`;for(const [cls,top]of detailScroll.get(lastRenderedView)||[]){const node=[...dock.querySelectorAll('.business-body,.business-evidence,.auction-bidding,.bid-scroll')].find(node=>node.className===cls);if(node)node.scrollTop=top;}

  }

  function celebrateHolding(id){
    renderPortfolio();
    requestAnimationFrame(()=>{
      const row=[...portfolioCard.querySelectorAll('.portfolio-holding')].find(row=>row.dataset.businessId===id);
      if(!row)return;
      // Reveal only the two portfolio scrollers; never scroll the document or camera.
      for(const box of [row.parentElement,portfolioCard]){const a=row.getBoundingClientRect(),b=box.getBoundingClientRect();if(a.top<b.top)box.scrollTop-=b.top-a.top;else if(a.bottom>b.bottom)box.scrollTop+=a.bottom-b.bottom;}
      const a=row.getBoundingClientRect(),b=portfolioCard.getBoundingClientRect(),x=a.left+a.width*.55,y=Math.max(b.top+18,Math.min(a.top+a.height*.6,b.bottom-12));
      const burst=el('div',undefined,'win-confetti');burst.setAttribute('aria-hidden','true');burst.dataset.businessId=id;burst.style.left=`${x}px`;burst.style.top=`${y}px`;
      dock.dataset.lastWinAnchor=id;
      for(let n=0;n<64;n++){const piece=el('i'),angle=n*2.39996,spread=60+(n%9)*13,dx=Math.sin(angle)*spread;piece.style.setProperty('--dx',`${dx}px`);piece.style.setProperty('--peak-x',`${dx*.58}px`);piece.style.setProperty('--peak-y',`${-Math.min(y-12,95+(n%8)*19)}px`);piece.style.setProperty('--fall-y',`${45+n%7*13}px`);piece.style.setProperty('--spin',`${(n%2?1:-1)*(280+n*13)}deg`);piece.style.background=['#e2c46d','#8ac5a4','#f0e7c8','#b7c5dd'][n%4];piece.style.animationDelay=`${n%5*.018}s`;piece.style.width=`${5+n%3}px`;burst.append(piece);}
  ...
  return {playerWin(lot){if(celebrated.has(lot.id)||lot.status!=='funded'||!getTown().investors.find(i=>i.id===lot.investorId)?.human)return;celebrated.add(lot.id);dock.dataset.playerWinCount=String(celebrated.size);dock.dataset.lastWinningLot=lot.id;onWin?.();if(matchMedia('(prefers-reduced-motion: reduce)').matches)return;celebrateHolding(lot.businessId);},select(id){if(id?.startsWith('business:')){const pid=id.slice(9),p=getTown().places.find(p=>p.id===pid);if(p?.investorId&&!getTown().businesses[pid]){const i=getTown().investors.find(i=>i.id===p.investorId);if(i){view={type:'investor',id:i.id};shown=true;collapsed=false;}}else if(getTown().businesses[pid]){view={type:'business',id:pid};shown=true;collapsed=false;}}else if(getTown().investors.some(i=>i.id===id)){view={type:'investor',id};shown=true;collapsed=false;}render();},renderDetail(){render();return false;},enhance(){const section=[...document.querySelectorAll('#inspector .section')].find(s=>s.querySelector('h3')?.textContent==='Businesses');if(!section)return;for(const label of section.querySelectorAll('.bar-label')){const p=getTown().places.find(p=>label.textContent.startsWith(p.name+' · '));if(!p)continue;const link=button(label.textContent,()=>openBusiness(p.id),'business-link');label.replaceChildren(link);}const firms=el('div',undefined,'section investment-firms');firms.append(el('h3','Investment firms'));for(const i of getTown().investors){const f=portfolio(getTown(),i),row=el('div',undefined,'firm-row');row.dataset.investorId=i.id;row.append(button(i.name,()=>{view={type:'investor',id:i.id};shown=true;collapsed=false;render();},'business-link'),el('span',`Real ${signedMoney(f.realized)} · Unrl ${signedMoney(f.unrealized)}`,'firm-gains'));row.lastChild.title='Realized / unrealized investment gains';firms.append(row);}section.after(firms);},pitchWaiting(){render();},async marketUpdate(){const t=getTown();if(!t.finance.playerActive)return;const lot=t.finance.offers.find(o=>o.id===t.finance.active);if(lot?.playerDeclined)return;if(lot&&lastFocused!==lot.id){view={type:'business',id:lot.businessId,lotId:lot.id};shown=true;lastFocused=lot.id;focusBusiness(lot.businessId);}render();if(lot)await new Promise(r=>setTimeout(r,t.finance.playerActive?120:70));},humanTurn(lot,investor){return new Promise(resolve=>{pending={lot,investor,resolve};render();});}};
